...
describe('for ExtensionHost', () => {
  it('runs extension setup and registers multiple module sessions', async () => {
    ...
    expect(session.extension.id).toBe('nekoai-extension-test')
  ...
  it('rejects defineExtension entrypoint ids that do not match the manifest id', async () => {
    const host = new ExtensionHost()
    const extension = defineExtension({
      id: 'nekoai-extension-entrypoint-id',
      async setup() {},
    })
    ...
    await expect(host.startExtension(extension, {
      manifest: {
        apiVersion: 'v1',
        kind: 'manifest.extension.airi.moeru.ai' as const,
        id: 'nekoai-extension-manifest-id',
        permissions: {},
        entrypoints: {},
      },
    })).rejects.toThrow(
      'Extension entrypoint id `nekoai-extension-entrypoint-id` must match manifest id `nekoai-extension-manifest-id`.',
    )
  })
  ...
  it('cleans up extension kit resources registered before setup failure', async () => {
    ...
    const extension = defineExtension({
      id: 'nekoai-extension-cleanup-failure',
      async setup(ctx) {
    ...
    await expect(host.startExtension(extension, {
      manifest: {
        ...
        permissions: {
          ...
          resources: [
            { key: 'proj-nekoai:plugin-sdk:resources:kits:kit.cleanup-failure:bindings', actions: ['write'] },
          ],
        },
        entrypoints: {},
      },
  ...
  it('cleans up module-scoped kit resources when the module is disposed', async () => {
    ...
    const permissions: ModulePermissionDeclaration = {
      ...
      resources: [
        { key: 'proj-nekoai:plugin-sdk:resources:kits:kit.module-dispose:bindings', actions: ['write'] },
      ],
    }
    const extension = defineExtension({
      id: 'nekoai-extension-module-dispose',
      async setup(ctx) {
  ...
  it('lets extension setup use granted kits without registering a module', async () => {
    ...
    let observed = ''
    const extension = defineExtension({
      id: 'nekoai-extension-direct-kit',
      async setup(ctx) {
        const client = await ctx.kits.use(kit)
    ...
    expect(observed).toContain('nekoai-extension-direct-kit:')
  ...
  it('denies extension-scoped kit use when the extension grant does not allow the kit', async () => {
    ...
    const extension = defineExtension({
      id: 'nekoai-extension-direct-kit-denied',
      async setup(ctx) {
  ...
  it('denies extension-scoped kit use when host permission resolver narrows the manifest grant', async () => {
    ...
    const extension = defineExtension({
      id: 'nekoai-extension-direct-kit-resolver-denied',
      async setup(ctx) {
        const result = await ctx.kits.tryUse(kit)
        expect(result.ok).toBe(false)
        if (!('reason' in result)) {
  ...
  it('does not let persisted grants override a later permission resolver decision', async () => {
    ...
    const manifest = {
      apiVersion: 'v1',
      kind: 'manifest.extension.airi.moeru.ai' as const,
      id: 'nekoai-extension-direct-kit-persisted-revoked',
      permissions: {
  ...
  it('lets module-scoped kit use inherit the extension grant when module permissions are omitted', async () => {
    ...
    const extension = defineExtension({
      id: 'nekoai-extension-module-inherited-grant',
      async setup(ctx) {
  ...
  it('denies module-scoped kit use when host permission resolver narrows the extension grant', async () => {
    ...
    const extension = defineExtension({
      id: 'nekoai-extension-module-kit-resolver-denied',
      async setup(ctx) {
        const module = await ctx.modules.register({
          id: 'module-a',
          permissions: {
  ...
  it('lets extension setup watch kit availability without registering a module', async () => {
    ...
    const extension = defineExtension({
      id: 'nekoai-extension-direct-kit-watch',
      async setup(ctx) {
  ...
  it('disposes extension-scoped kit availability watchers with the extension session', async () => {
    ...
    const observed: boolean[] = []
    const extension = defineExtension({
      id: 'nekoai-extension-direct-kit-watch-dispose',
      async setup(ctx) {
        ctx.kits.watch(kit, (availability) => {
          observed.push(availability.available)
        })
  ...
  it('disposes module-scoped kit availability watchers with the module scope', async () => {
    ...
    const extension = defineExtension({
      id: 'nekoai-extension-module-kit-watch-dispose',
      async setup(ctx) {
        const module = await ctx.modules.register({
          id: 'module-a',
          permissions: {
  ...
  it('rejects duplicate module ids without replacing the registered module', async () => {
    ...
    const disposed: string[] = []
    const extension = defineExtension({
      id: 'nekoai-extension-duplicate-module',
      async setup(ctx) {
  ...
  it('waits for async extension module cleanup while stopping a defineExtension session', async () => {
    ...
    const extension = defineExtension({
      id: 'nekoai-extension-async-stop-cleanup',
      async setup(ctx) {
        const module = await ctx.modules.register({ id: 'module-a' })
  ...
  it('denies kit use when module permissions exceed the extension grant ceiling', async () => {
    ...
    const extension = defineExtension({
      id: 'nekoai-extension-kit-denied',
      async setup(ctx) {
        const module = await ctx.modules.register({
          id: 'module-a',
          permissions: {
...
describe('for FileSystemLoader', () => {
  const testPermissions: ModulePermissionDeclaration = {
    apis: [
      { key: 'proj-nekoai:plugin-sdk:apis:protocol:capabilities:wait', actions: ['invoke'] },
      { key: 'proj-nekoai:plugin-sdk:apis:protocol:resources:providers:list-providers', actions: ['invoke'] },
    ],
    resources: [
      { key: 'proj-nekoai:plugin-sdk:apis:protocol:resources:providers:list-providers', actions: ['read'] },
    ],
    capabilities: [
      { key: 'proj-nekoai:plugin-sdk:apis:protocol:resources:providers:list-providers', actions: ['wait'] },
    ],
  }
