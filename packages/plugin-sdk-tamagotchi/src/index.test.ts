...
import type { HostDataRecord } from '@nekotech/plugin-sdk/plugin-host'

import type { ToolKitRuntime } from './tools'

import { DisposableStore } from '@nekotech/plugin-sdk'
...
describe('plugin-sdk-tamagotchi', () => {
  ...
  it('exposes gameletKit as a module-scoped kit client', async () => {
    ...
    const client = gameletKit.createClient(createGameletRuntime({
      extensionId: 'nekoai-extension-chess',
      sessionId: 'session-1',
      moduleId: 'chess',
  ...
  it('routes createGamelet handle orchestration calls through the host gamelet runtime', async () => {
    ...
    await handle.configure({ nekoaiSide: 'black' })
    await handle.request({ action: 'snapshot' })
    await handle.close()
    await expect(handle.isOpen()).resolves.toBe(true)

    expect(open).toHaveBeenCalledWith('chess:board', { mode: 'new' })
    expect(configure).toHaveBeenCalledWith('chess:board', { nekoaiSide: 'black' })
  ...
  it('reports a clear error when createGamelet orchestration methods run without a host runtime', async () => {
    ...
    await expect(handle.open()).rejects.toThrow('gameletKit requires a host gamelet orchestration runtime.')
    await expect(handle.configure({ nekoaiSide: 'black' })).rejects.toThrow('gameletKit requires a host gamelet orchestration runtime.')
  ...
  it('exposes toolKit as a module-scoped kit client without a gamelet runtime', async () => {
    ...
    await client.registerToolsetPrompt({
      id: 'chess-toolset',
      prompt: {
        id: 'nekoai-plugin-game-chess.prompt',
  ...
  /**
   * @example
   * expect(registerToolsetPrompt).toHaveBeenCalledWith({ id: 'nekoai-plugin-game-chess.prompt', prompt: expect.any(Object) })
  ...
  it('stores, invokes, and removes module-scoped Tamagotchi tools', async () => {
    ...
    registry.register({
      ...
      ownerExtensionId: 'nekoai-extension-chess',
      ownerModuleId: 'chess',
      tool: {
        id: 'play_chess',
        title: 'Play Chess',
    ...
    await expect(registry.invoke('nekoai-extension-chess', 'play_chess', { move: 'e4' })).resolves.toEqual({ ok: true })
    ...
    await expect(registry.invoke('nekoai-extension-chess', 'play_chess', {})).rejects.toThrow(
      'Tamagotchi extension tool not found: nekoai-extension-chess:play_chess',
    )
  })
  ...
  it('creates a gamelet helper with a generated id when omitted', async () => {
    ...
    const { module } = createGameletModuleRef({
      id: 'feature',
      extensionId: 'nekoai-extension-feature',
      sessionId: 'session-1',
      bind: registerBinding,
    })
