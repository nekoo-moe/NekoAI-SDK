...
1. **Static Validation**: At load time, the host checks whether the requested capability is declared in the plugin's `plugin.json` (or legacy `neko-plugin.json`).
...
1. Host discovers `plugin.json` (or legacy `neko-plugin.json`) and parses requested capabilities.
